...
{
  double hz = 1.0;
  // 读取 名为“hz”的port中 的value 并赋值给 hz
  getInput("hz", hz);
  period_ = 1.0 / hz;
}
...
BT::NodeStatus RateController::tick()
{
  ...
  // 'til completion
  // RateController继承自BT::DecoratorNode， child_node_ 是BT::DecoratorNode的成员变量
  // 装饰节点(decorator) 有且仅有 一个子节点(child_node_)
  if (first_time_ || (child_node_->status() == BT::NodeStatus::RUNNING) ||
    seconds.count() >= period_)
  {
    first_time_ = false;
    const BT::NodeStatus child_state = child_node_->executeTick();
    // 子节点状态 决定了 RateController 的状态
    switch (child_state) {
