...
{
// RecoveryServer 中有 2 + 2 + default_ids_.size() + 2 + 1个参数，其名字分别为 
// “ costmap_topic ”： 用于订阅 名为 "local_costmap/costmap_raw" 的话题消息
// “ footprint_topic ”： 用于订阅 名为 “local_costmap/published_footprint” 的话题消息

// “ default_ids_[i] + ".plugin" ”： 一系列恢复器id 对应的 插件（是一系列子类，继承nav2_core中的虚基类）

// 这两个frame有什么用？
// " global_frame "： 默认的 全局坐标系（global_frame）是 odom
// " robot_base_frame "：默认的 机器人坐标系（robot_base_frame）是 base_link
...
RecoveryServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  // get_node_base_interface() 返回节点的内部NodeBaseInterface实现。
