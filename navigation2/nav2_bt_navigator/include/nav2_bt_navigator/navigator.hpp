...
public:
  ...
  bool isNavigating()
  {
    ...
    // current_navigator_ 不为空时，返回true
 ...
 */
// 接口(interface)类 没有函数定义，必有类来继承此类
// NavigateToPoseNavigator类 和 NavigateThroughPosesNavigator类 继承了 此类
...
public:
    ...
    std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
  {
    ...
    // 使用 行为树 执行 action的 action server
    // Behavior Tree的执行 和 取消 是通过action实现的，BT由多个LifecycleNode构成
