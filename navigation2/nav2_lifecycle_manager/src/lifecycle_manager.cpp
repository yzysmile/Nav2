...
: Node("lifecycle_manager")
{
  ...
  // 实例化两个service 并绑定其回调函数于其中，并将其回调函数放置于回调组中 
  ...
  init_timer_ = this->create_wall_timer(
    0s,
    [this]() -> void {
      // 只触发一次timer中的Lambda函数，不间隔时间触发
      init_timer_->cancel();
       
       // 为被管理的 LifecycleNodes 创建ServiceClients
      createLifecycleServiceClients();

      // 若是自启动， 则被管理的 LifecycleNodes执行 成员函数startup(),
      // 以实现则被管理的 LifecycleNodes 处于 activate 状态
      if (autostart_) {
        init_timer_ = this->create_wall_timer(
          0s,
          [this]() -> void {
  ...
  // 添加回调组到一个执行器中
  // 使用此方法 添加回调组 不会将其节点(node)与此执行器相关联
  executor->add_callback_group(callback_group_, get_node_base_interface());
   
  // 本质是在一个子线程中 spin这个executor,这个executor中有回调函数
  // 即回调函数在一个子线程执行
  ...
  std::shared_ptr<ManageLifecycleNodes::Response> response)
{
  switch (request->command) {
    case ManageLifecycleNodes::Request::STARTUP:
      RCLCPP_INFO(get_logger(), "Request::STARTUP##########");
...
LifecycleManager::createBondConnection(const std::string & node_name)
{
  ...
  // 若在bond_map_中不存在 key为“node_name”的节点，在对应的value中创建 bond
  // bond -- a mechanism for checking when another process has terminated.
...
LifecycleManager::changeStateForAllNodes(std::uint8_t transition)
{
  ...
  {
    // 遍历被管理的LifecycleNodes 以改变他们的状态，其本质是通过 该Node发送 request的方式进行的
    // 此Node要发送多个request 以改变多个的LifecycleNodes的 状态，
    // 故nav2_util/service_client.hpp 以回调组的形式实现客户端
    for (auto & node_name : node_names_) {
