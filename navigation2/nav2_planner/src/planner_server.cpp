...
{
// PlannerServer 中有 2 + default_ids_.size() 个参数，其名字分别为 
// “ planner_plugins ”： 其值为 一系列规划器 id 
// “ expected_planner_frequency ”： 规划器的规划频率，默认值为1hz
// “ default_ids_[i] + ".plugin" ”： 一系列规划器id 对应的 插件（是一系列子类，继承nav2_core中的虚基类）
PlannerServer::PlannerServer()
: nav2_util::LifecycleNode("nav2_planner", "", true),
  // 初始化pluginlib::ClassLoader类对象 gp_loader_("接口类命名空间"，“接口类命名空间::接口类”)
  ...
  costmap_(nullptr)
{
  ...
  // 这些参数的实际获取是通过参数名称 在 bringup/launch/params 中 .yaml文件中
  ...
  if (planner_ids_ == default_ids_) {
    for (size_t i = 0; i < default_ids_.size(); ++i) {
      ...
      // 默认情况下名为“GridBased.plugin”的参数的值是 nav2_navfn_planner/NavfnPlanner
    }
  }
...
// 凡是 一个LifecycleNode 其私有变量均在on_configure函数中 初始化 
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      // 默认情况 planner_ids_[0] 为 GridBased
      // 默认情况 planner_types_[0] 为 nav2_navfn_planner/NavfnPlanner
      planner_types_[i] = nav2_util::get_plugin_type_param(
        node, planner_ids_[i]);
      // 以加载插件的形式进行实例化
      // 默认情况下 planner 是 class NavfnPlanner 类对象(用于全局路径规划)
      // class NavfnPlanner类 公有继承 nav2_core::GlobalPlanner虚基类，故 planner 也是 nav2_core::GlobalPlanner类对象
      // planner_types_[i] 应该是一个 "插件类命名空间::插件类", 比如 nav2_navfn_planner::NavfnPlanner
      ...
      // 默认情况调用 class NavfnPlanner 类中 重写(override)的 configure(...)函数
      planner->configure(node, planner_ids_[i], tf_, costmap_ros_);

      // 将各种算法的planner 放入容器变量planners_ 是一个std::unordered_map
  ...
  // action的client 请求 单个目标点 后， action的server 调用 回调函数 computePlan
...
// LifecycleNode 在on_activate函数中 激活 发布路径的publisher 和 路径规划的server
nav2_util::CallbackReturn
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    result->path = getPlan(start, goal_action_server_pose_pose, goal->planner_id);
