 ...
 */
// 此模板类 是对service（client,server）中 的client 以回调组的形式发送request 再一次封装在 类成员函数invoke中
// 客户端以以回调组的形式发送request 目的是 可以同时对不同名称的node发送request吧
...
public:
  ...
  */
  ...
  {
    // 在node中创建回调组，
     // 第二个参数 true 表示以 add_node的方式绑定 node
     //          false 表示不以 add_node的方式绑定 node吧
    callback_group_ = node_->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive,
      false);

     // 不以add_node方式绑定node， 即node和callback_group是平行关系，
     // 则需要在执行器中添加callback_group, 并指定该callback_group在哪个node中被调用
     // ServiceClient类 在client端创建回调组，可以同时对不同名称的node发送request
     // 之前我们都是在service端创建回调组
